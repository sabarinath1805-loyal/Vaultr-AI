...
// vaultrApi resolves the auth header through the module-level Supabase client,
...
describe("VaultrApiError / isMfaRequiredError", () => {
    it("carries status and code, defaulting code to null", () => {
        const withCode = new VaultrApiError({
            message: "nope",
            status: 403,
            code: "mfa_verification_required",
        });
        expect(withCode.name).toBe("VaultrApiError");
        expect(withCode.status).toBe(403);
        expect(withCode.code).toBe("mfa_verification_required");

        const withoutCode = new VaultrApiError({ message: "nope", status: 500 });
    ...
    it("recognizes exactly the 403 + mfa_verification_required combination", () => {
        ...
        expect(
            isMfaRequiredError(
                new VaultrApiError({ message: "x", status: 403, code: "other" }),
            ),
        ).toBe(false);
        expect(
            isMfaRequiredError(
...
describe("apiRequest plumbing (via thin wrappers)", () => {
    ...
    it("maps a JSON error body to a VaultrApiError with code and detail", async () => {
        fetchMock.mockResolvedValue(
            jsonResponse(
        ...
        expect(error).toBeInstanceOf(VaultrApiError);
        const apiError = error as VaultrApiError;
...
describe("blob requests (exportAccountData)", () => {
    ...
    it("throws a VaultrApiError on failure", async () => {
        fetchMock.mockResolvedValue(
...
// Error carrying the raw response text instead of VaultrApiError.
// ---------------------------------------------------------------------------
...
describe("multipart upload endpoints", () => {
    ...
    it("upload failures throw a plain Error with the response text, not VaultrApiError", async () => {
        ...
        expect(error).not.toBeInstanceOf(VaultrApiError);
