...
const BACKEND_OUT = path.join(OUTPUT_ROOT, "backend/src/lib/systemWorkflows.ts");
const LANDING_OUT = path.join(OUTPUT_ROOT, "landing/app/generated-workflows.ts");
...
function readSkillFile(filePath) {
  ...
  return {
    ...
    body: afterClose.trimStart().trimEnd(),
...
function main() {
  ...
  try {
    ...
    const actualRef = execFileSync("git", ["-C", WORKFLOWS_DIR, "rev-parse", "HEAD"], {
      encoding: "utf8",
      stdio: ["ignore", "pipe", "ignore"],
    }).trim();
    if (actualRef.toLowerCase() !== EXPECTED_SOURCE_REF.toLowerCase()) {
      fail(`Workflow source ref mismatch: expected ${EXPECTED_SOURCE_REF}, found ${actualRef}`);
    }
  } catch (error) {
    if (error.message?.startsWith("Workflow source ref mismatch:")) throw error;
    fail(`Workflow source directory is not a Git checkout at the pinned ref: ${relative(WORKFLOWS_DIR)}`);
  }
  for (const collection of WORKFLOW_COLLECTIONS) {
    const collectionDir = path.join(WORKFLOWS_DIR, collection.directory);
