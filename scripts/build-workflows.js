...
function readWorkflow(category, workflowDir) {
  ...
  const slashTrigger = metadata["mike-slash-trigger"];
  if (slashTrigger !== undefined) {
    assertString(slashTrigger, `${label}.metadata.mike-slash-trigger`);
    if (category !== "assistant") {
      fail(`${label}.metadata.mike-slash-trigger is only supported for assistant workflows`);
    }
    if (!SLASH_TRIGGER_PATTERN.test(slashTrigger)) {
      fail(
        `${label}.metadata.mike-slash-trigger must start with "/" and contain only lowercase letters, numbers, and hyphens`,
      );
    }
  }
  ...
  const normalizedMetadata = {
    ...
    jurisdictions: metadata.jurisdictions
      ...
      .map((item) => item.trim())
      .filter(Boolean),
    ...(slashTrigger ? { slash_trigger: slashTrigger } : {}),
  };
