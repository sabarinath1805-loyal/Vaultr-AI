  ...
  return (
    ...
    >
      ...
      <span
        style={{
          fontSize: '16px',
          lineHeight: isMax ? '28px' : '24px',
          marginLeft: '-2px',
          fontWeight: isMax ? 500 : 400,
          color: isMax ? 'var(--text-primary)' : 'var(--text-secondary)',
        }}
      >{label}</span>
      <span className={`lex-thinking-dots${isMax ? " lex-deep-analysis-dots" : ""}`} aria-hidden="true" style={{ lineHeight: isMax ? '28px' : '24px' }}>
        <span>●</span>
        <span>●</span>
        <span>●</span>
      </span>
