...
/**
 * Configured @vaultr/api-client barrel for the Word add-in — the single place the
 * shared typed client is wired to the add-in's Office session. Mirrors
 * apps/web/src/app/lib/vaultrApi.ts, but its auth comes from ../auth/session
 * (OfficeRuntime.storage-backed) instead of Supabase's browser SDK.
 *
 * Components import API functions FROM THIS MODULE (not from "@vaultr/api-client"
 * directly) so that importing any of them runs the side-effecting
 * configureVaultrApiClient() below before the first request leaves.
 */
import { configureVaultrApiClient } from "@vaultr/api-client";
import type { Document } from "@vaultr/core";
...
configureVaultrApiClient({
  baseUrl: BASE_URL,
  getAuthHeaders,
  fetchImpl: fetchWithRefresh,
});

export * from "@vaultr/api-client";

/**
 * List a project's documents (GET /projects/:id/documents). @vaultr/api-client
